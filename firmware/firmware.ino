...
void loop()
{
    ...
    while (true)
    {
        ...
        if (counters_trigger_state)
        { // Atleast one pulse has been detected
            break;
        }
    }
    ...
    {
        counters_trigger_state = get_counters_trigger_state();
        if (counters_trigger_state == 0b00000111)
        { // All have been triggered
    ...
    if (DEBUG_PRINTOUT)
    {
        ...
        Serial.print((double)three_pulses_time - (double)trigger_time);
        Serial.println(" us for three pulses to arrive");
        read_counters.DEBUG_print();
        Serial.println();
    }
